...
Flow per half-inning:
  pitcher score (49%) + batter score (27%) + park/weather (13%)
  + damage/speed (4%) + lineup confirmation (7%)
  → composite [0,1]
  → logistic sigmoid → P(no run this half) in [0.60, 0.91]

Weights concentrated on the pitcher (proven signal) after a 3-month backtest
showed the model compresses real game-to-game differences ~5x; the near-noise
damage/speed block was cut from 10%→4% to reduce that variance collapse.
